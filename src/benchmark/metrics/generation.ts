...
export interface GenerationMetrics {
    ...
    citationCoverage: number;     // % of papers cited
 ...
 */
export function calculateCitationDensity(text: string): number {
 ...
 */
export function calculateCitationCoverage(text: string, totalPapers: number): number {
    if (totalPapers === 0) return 1;
 ...
 */
export function calculateRouge1(generated: string, reference: string): {
    precision: number;
    recall: number;
    f1: number;
} {
 ...
 */
export function detectHallucinatedCitations(text: string, totalPapers: number): number[] {
 ...
 */
export function aggregateGenerationMetrics(results: GenerationResult[]): GenerationMetrics {
    if (results.length === 0) {
        return {
            avgLength: 0,
            citationDensity: 0,
