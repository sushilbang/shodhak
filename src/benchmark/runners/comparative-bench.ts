...
import { calculateKeywordCoverage } from '../metrics/retrieval';
import {
...
import {
    ...
    saveJsonReport,
    getComparativeConfig
} from '../utils';
import groundTruth from '../datasets/ground-truth.json';

interface LLMConfig extends LLMClientConfig {
    name: string;
}
...
interface ComparativeBenchmarkReport {
    timestamp: string;
    testQueries: number;
...
export async function runComparativeBenchmark(
    ...
    const {
        queriesLimit = 3,
        papersPerQuery = 5,
        configs = LLM_CONFIGS.filter(c => c.apiKey),
        verbose = true
    } = options;

    const queries = groundTruth.queries.slice(0, queriesLimit);

    printHeader('COMPARATIVE BENCHMARK', 45);
    ...
    const retrievalCoverages: number[] = [];
    ...
    for (const q of queries) {
        ...
        for (const config of configs) {
            ...
            if (result.error) {
                ...
                const citDensity = calculateCitationDensity(result.content);
                const citCoverage = calculateCitationCoverage(result.content, papers.length);
                const hallucinations = detectHallucinatedCitations(result.content, papers.length);
    ...
    // Compile results
    const compiledResults: ComparisonResult[] = [];
    ...
    printHeader('COMPARISON RESULTS', 68);

    const tableColumns = [
        { header: 'Model', width: 25, align: 'left' as const },
        { header: 'Latency', width: 10, align: 'right' as const },
        { header: 'Length', width: 8, align: 'right' as const },
        { header: 'Cit.Dens', width: 10, align: 'right' as const },
        { header: 'Halluc', width: 8, align: 'right' as const }
    ];
...
function generateRankings(results: ComparisonResult[]): { metric: string; ranking: string[] }[] {
    const rankings: { metric: string; ranking: string[] }[] = [];
    ...
    const byLength = [...results]
        ...
        .sort((a, b) =>
            Math.abs(a.generation.avgLength - idealLength) -
...
function determineBestOverall(results: ComparisonResult[]): string {
...
if (require.main === module) {
    runComparativeBenchmark({ verbose: true })
        .then(report => {
            const outputPath = saveJsonReport('comparative-benchmark-report.json', report);
