...
class LLMService {
    constructor() {
        ...
        const reasoning = (0, llm_client_factory_1.createReasoningClient)();
        this.reasoningClient = reasoning.client;
        this.reasoningModel = reasoning.model;
        const fast = (0, llm_client_factory_1.createFastClient)();
...
Respond with ONLY the simple keyword query, nothing else.`,
        ...
        logger_1.logger.info('Refined query', { original: userQuery, refined: refinedQuery });
        return refinedQuery;
    }
    async generateClarifyingQuestions(userQuery) {
        const response = await this.fastClient.chat.completions.create({
            model: this.fastModel,
            max_tokens: 512,
            messages: [
...
Provide a concise summary.`,
    ...
    async generateLiteratureReview(papers, query) {
...
Write in academic style with clear paragraphs. Include citations for all claims.`,
        ...
        logger_1.logger.info('Generated literature review', {
            query,
...
Be specific and cite which paper you're referring to.`,
    ...
    async answerQuestions(question, papers) {
        const paperContext = papers
            .map((p, idx) => {
