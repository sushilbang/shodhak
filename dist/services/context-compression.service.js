...
exports.contextCompression = void 0;
...
class ContextCompressionService {
    constructor() {
        const { client, model } = (0, llm_client_factory_1.createReasoningClient)();
        this.client = client;
        this.model = model;
    }
    /**
    ...
    async maybeCompress(context) {
        ...
        if (alreadyCompressed >= messagesToCompress)
            return;
        ...
        if (oldMessages.length === 0)
            return;
        logger_1.logger.info('Compressing conversation history', {
        ...
        try {
            ...
            // Persist to DB
            await session_persistence_service_1.sessionPersistence.saveSummary(context.sessionId, summary, startIdx, endIdx, conversationSummary.tokenEstimate);
            for (const fact of facts) {
                await session_persistence_service_1.sessionPersistence.saveKeyFact(context.sessionId, fact.type, fact.content, fact.relatedPaperIndices);
            }
            // Delete compressed raw messages from DB
            await session_persistence_service_1.sessionPersistence.deleteMessagesByOrderRange(context.sessionId, startIdx, endIdx);
            logger_1.logger.info('Compression complete', {
                sessionId: context.sessionId,
            ...
            logger_1.logger.error('Context compression failed', { sessionId: context.sessionId, error: err });
        }
    }
    /**
     ...
     */
    buildCompressedContext(systemPrompt, context) {
        const messages = [];
    ...
    async extractKeyFacts(messages, context) {
        const conversationText = messages
...
Only extract genuinely important facts. Return valid JSON array only, no other text.`,
        ...
        try {
            ...
            logger_1.logger.warn('Failed to parse key facts from LLM response', { rawContent, error: err });
            return [];
        }
    }
}
exports.contextCompression = new ContextCompressionService();
