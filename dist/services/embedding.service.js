...
class EmbeddingService {
    constructor() {
        ...
        logger_1.logger.info('Embedding service initialized with OpenAI text-embedding-3-small');
    }
    ...
    async embedPaper(paper) {
        if (!paper.id) {
            logger_1.logger.error('Paper must have an ID to embed');
            return null;
        }
        const existing = await database_1.pool.query('SELECT * FROM paper_embeddings WHERE paper_id = $1', [paper.id]);
        ...
        const result = await database_1.pool.query(`INSERT INTO paper_embeddings (paper_id, embedding)
            VALUES ($1, $2)
            RETURNING id, paper_id, embedding, created_at`, [paper.id, JSON.stringify(embedding)]);
        logger_1.logger.info('Generated embedding for paper', {
            paperId: paper.id,
            title: paper.title,
    ...
    async embedPapers(papers) {
        ...
        for (const paper of papers) {
            try {
                ...
                logger_1.logger.error('Failed to embed paper', {
                    error,
                    paperId: paper.id,
                    title: paper.title,
                });
    ...
    async searchSimilarPapers(query, limit = 10) {
    ...
    cosineSimilarity(a, b) {
        if (a.length !== b.length) {
            logger_1.logger.warn('Embedding dimension mismatch', { a: a.length, b: b.length });
            return 0;
        }
        let dotProduct = 0;
        let normA = 0;
    ...
    async migrateEmbeddings() {
        const result = await database_1.pool.query('SELECT COUNT(*) as count FROM paper_embeddings');
        const count = parseInt(result.rows[0].count, 10);
        if (count === 0) {
            logger_1.logger.info('No embeddings to migrate');
            return;
        }
        // Check if any embedding has wrong dimensions
        const sample = await database_1.pool.query('SELECT embedding FROM paper_embeddings LIMIT 1');
        if (sample.rows.length > 0) {
            ...
            if (Array.isArray(embedding) && embedding.length !== this.dimensions) {
                logger_1.logger.info('Embedding dimension mismatch detected, re-embedding all papers', {
                    currentDimensions: embedding.length,
                    targetDimensions: this.dimensions,
                });
                await database_1.pool.query('TRUNCATE paper_embeddings');
                const papers = await database_1.pool.query('SELECT * FROM papers');
                ...
                logger_1.logger.info('Embedding migration complete', { papersReEmbedded: papers.rows.length });
            }
            else {
                logger_1.logger.info('Embeddings already at correct dimensions');
            }
        }
    }
    getProviderInfo() {
