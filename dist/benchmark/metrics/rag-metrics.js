...
exports.calculateFaithfulness = calculateFaithfulness;
exports.calculateAnswerRelevance = calculateAnswerRelevance;
...
exports.evaluateRAGWithPapers = evaluateRAGWithPapers;
exports.aggregateRAGMetrics = aggregateRAGMetrics;
// ============================================================================
// IMPLEMENTATION
// ============================================================================
/**
...
function extractClaims(text) {
    ...
    return sentences.filter(s => {
        ...
        if (lower.startsWith('i think') || lower.startsWith('i believe'))
            return false;
        // Exclude meta-statements
        if (lower.startsWith('this paper') && lower.includes('will'))
            return false;
        return true;
    });
}
 ...
 */
function isClaimSupported(claim, context, threshold = 0.3) {
 ...
 * ```
 */
function calculateFaithfulness(generatedAnswer, contextDocuments) {
 ...
 * ```
 */
function calculateContextPrecision(question, documents, relevanceScores // Optional explicit relevance judgments
) {
 ...
 * ```
 */
function calculateContextRecall(generatedAnswer, contextDocuments, groundTruthDocs) {
    ...
    let groundTruthRecall;
    if (groundTruthDocs && groundTruthDocs.length > 0) {
        const contextSet = new Set(contextDocuments.map(d => d.toLowerCase().slice(0, 100)));
        const matchedGroundTruth = groundTruthDocs.filter(gt => contextSet.has(gt.toLowerCase().slice(0, 100))).length;
 ...
 * ```
 */
function evaluateRAG(question, generatedAnswer, contextDocuments, options = {}) {
    const { groundTruthDocs, relevanceScores, weights = {
        faithfulness: 0.30,
    ...
    // Weighted average
    const weightedScore = weights.faithfulness * faithfulness.score +
 ...
 */
function evaluateRAGWithPapers(question, generatedAnswer, papers, options) {
    // Extract text context from papers
    const contextDocuments = papers.map(p => `${p.title}. ${p.abstract || ''}`);
